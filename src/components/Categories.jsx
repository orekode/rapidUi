  ...
  return (
    <Scroll.Side>
        {(data && data.data && data.data.map) && data.data.map((item, index) =>
          <Link to={`/shop/category/${encrypt(item.id)}`} key={index} className="w-[180px] h-[220px] relative group active:scale-90 transition-all duration-300">
...
export const PopularCircle = () => {
  ...
  return (
    <Scroll.Side>
        {(data && data.data && data.data.map) && data.data.map((item, index) =>
          <Link to={`/shop/category/${encrypt(item.id)}`} key={index} className="text-center">
